...
import { Grid, createFilterOptions, Box, Stack, Typography, Button, TextField, RadioGroup, IconButton, FormControlLabel, Radio } from "@mui/material";
...
function Form(props) {
  ...
  //   try {
  //     await createPatent(values);
  //     reset(); // Menghapus nilai input setelah membuat data
  //     await navigate(`/app/patent`)
  //   } catch (error) {
  //     console.log(error);
  //   }
  // }
  ...
  const onSubmit = async () => {
    ...
    try {
      if (id !== "register") {
        ...
        await createPatent({ ...datasend, username: "", password: "" });
      }
      await navigate(`/app/patent`);
    } catch (err) {
      alert(err);
