...
function DetailTable(props) {
  // context
  const { filterPatents } = useContext(PatentContext)
    ...
    <>
      <TableContainer sx={tableContainer}>
        <Table stickyHeader sx={tableXS} aria-label='simple table'>
          <TableHead />
          {filterPatents?.length ? (
            <TableBody>
              {_.sortBy(filterPatents, ['_id'])
                  ...
                  return (
                    <TableRow hover role='checkbox' tabIndex={-1} key={row._id}>
                      ...
                      <TableCell align='left'>
                        <Typography>{row.pateningEvent}</Typography>
                      </TableCell>
                      <TableCell align='left'>
                        <Typography>{moment(row.pateningEventDate).format('DD-MM-YYYY mm:ss')}</Typography>
                      </TableCell>
                      <TableCell align='left'>
